...
pub fn parse_instruction(input: TokenStream) -> TokenStream {
    ...
    // 生成账户解析
    let account_parsing = instruction_def
        .accounts
        .iter()
        .enumerate()
        .map(|(i, account)| {
    ...
    // 生成指令检查逻辑
    let instruction_parsing = if has_data_fields {
        let data_struct_name = quote::format_ident!("{}Data", name);
        // 生成数据字段解析
    ...
    // 为 from_full_data 生成指令解析逻辑（不需要账户信息）
        ...
        impl #name {
            ...
            pub fn from(tx: &grpc_client::TransactionFormat) -> Vec<Self> {
                ...
                for instruction in tx.transation.message.instructions() {
                    if let Some(mut parsed) = Self::check_instruction(instruction, accounts) {
                        parsed.slot = tx.slot;
                        results.push(parsed);
                    }
                }
                    ...
                    if let Some(ref inner_instructions) = meta.inner_instructions {
                        for inner_ix_group in inner_instructions {
                            for inner_instruction in &inner_ix_group.instructions {
                                if let Some(mut parsed) = Self::check_instruction(&inner_instruction.instruction, accounts) {
                                    parsed.slot = tx.slot;
                                    results.push(parsed);
                                }
                            }
